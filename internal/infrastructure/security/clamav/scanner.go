...
func (c *Client) Scan(ctx context.Context, data []byte) (*ScanResult, error) {
	...
	for i := 0; i < len(data); i += chunkSize {
		...
		sizeBytes := []byte{
			byte((size >> bitShift24) & 0xFF), // #nosec G115 -- safe cast after shift and mask
			byte((size >> bitShift16) & 0xFF), // #nosec G115 -- safe cast after shift and mask
			byte((size >> bitShift8) & 0xFF),  // #nosec G115 -- safe cast after shift and mask
			byte(size & 0xFF),                 // #nosec G115 -- safe cast after mask
		}
