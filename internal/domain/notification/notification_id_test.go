...
func TestNotificationID(t *testing.T) {
	...
	t.Run("ParseNotificationID parses valid UUID", func(t *testing.T) {
		u := uuid.New().String()
		id, err := notification.ParseNotificationID(u)
		assert.NoError(t, err)
		assert.Equal(t, u, id.String())
	})
	...
	t.Run("Equals compares correctly", func(t *testing.T) {
		...
		id2, _ := notification.ParseNotificationID(id1.String())
