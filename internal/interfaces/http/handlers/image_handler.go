...
func (h *ImageHandler) Routes(rateLimiterCfg *middleware.RateLimiterConfig) chi.Router {
	r := chi.NewRouter()

	// All routes require authentication - applied at router level
	// Upload route has special rate limiting
	if rateLimiterCfg != nil {
		r.With(middleware.UploadRateLimiter(*rateLimiterCfg)).Post("/", h.Upload)
	} else {
		r.Post("/", h.Upload)
	}
	r.Get("/", h.List)
