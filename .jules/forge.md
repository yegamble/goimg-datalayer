...
## 2026-02-05 - PostgreSQL Health Check User
**Issue:** PostgreSQL container in CI was failing health checks with 'role "root" does not exist' fatal errors.
**Root Cause:** The GitHub Actions `--health-cmd` for the PostgreSQL service was running `pg_isready` as the default `root` user, which is not set up in the database container, causing connection rejections and failing the health check before migrations could run.
**Fix:** Modified the health check command to explicitly specify the user: `--health-cmd "pg_isready -U goimg_test"`.
