...
## 2026-01-30 - Pinned Security Tool Installers
**Issue:** `security.yml` installed `gitleaks` via unverified tarball download and `grype` via an installer script from the `main` branch. This posed a supply chain risk and potential for non-reproducible builds.
**Root Cause:** Convenience scripts were used without version pinning or checksum verification.
**Fix:** Switched `gitleaks` to `go install` (leveraging module proxy verification) and pinned the `grype` installer script to the release tag version.
