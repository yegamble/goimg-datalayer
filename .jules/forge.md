...
## 2026-01-23 - Broken Trivy Action
**Issue:** `aquasecurity/trivy-action` pipeline broken with 'Unable to resolve action aquasecurity/setup-trivy' or binary download errors.
**Root Cause:** Older versions of `aquasecurity/trivy-action` (like 0.28.0) referenced an invalid or deleted `setup-trivy` tag.
**Fix:** Updated `aquasecurity/trivy-action` to at least v0.34.0 (pinned to stable commit hash `c1824fd6edce30d7ab345a9989de00bbd46ef284`).
