...
## 2026-01-23 - PostgreSQL Health Check Failure due to Missing User Role
**Issue:** CI integration test and build stages would fail intermittently with 'role does not exist' initialization errors for PostgreSQL services when running health checks.
**Root Cause:** The `pg_isready` command in docker-compose and github action files did not explicitly pass the username (`-U`), so it defaulted to the process owner (often `postgres` or `root`), whereas the initial `POSTGRES_USER` configured via environment variables was `goimg_test`.
**Fix:** Explicitly define the correct user with the `-U` flag in health checks using `pg_isready -U goimg_test` to prevent the tool from querying a non-existent default role.
