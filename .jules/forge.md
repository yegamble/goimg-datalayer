...
## 2026-04-18 - Pinned Production Image Versions
**Issue:** `docker-compose.prod.yml` was using `:latest` and `:stable` tags for external services like Certbot, Prometheus, Grafana, IPFS, and ClamAV.
**Root Cause:** The production compose file lacked pinned semantic versions, which violates infrastructure best practices and risks unexpected production breakages from upstream changes.
**Fix:** Pinned external services to specific semantic versions (e.g., `prom/prometheus:v2.53.5`) while leaving internal `goimg-*` images as `:latest` to support local deployment documentation workflows.
