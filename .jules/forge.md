...
## 2026-01-23 - Parallel Docker Builds
**Issue:** `docker-build` job in CI was running sequentially for API and Worker images, increasing total CI time.
**Root Cause:** Sequential steps in a single job.
**Fix:** Implemented GitHub Actions matrix strategy to run Docker builds in parallel, reducing job duration.
