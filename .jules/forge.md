...
## 2026-01-23 - Trivy Action Missing Setup Action
**Issue:** Security workflow failing due to 'Unable to resolve action aquasecurity/setup-trivy'.
**Root Cause:** The pipeline used an outdated version of `aquasecurity/trivy-action` (`0.28.0`). Older versions reference deleted or invalid `setup-trivy` action tags.
**Fix:** Updated `aquasecurity/trivy-action` to `v0.36.0` (pinned via SHA `a9c7b0f06e461e9d4b4d1711f154ee024b8d7ab8`) which correctly handles the setup dependencies.
