...
## 2026-02-12 - Unpinned Docker Images in Production Compose
**Issue:** `docker-compose.prod.yml` was using `:latest` and `:stable` tags for external services like Certbot, ClamAV, IPFS, Prometheus, and Grafana.
**Root Cause:** The production compose file lacked precise version pinning, which can cause unexpected breakage in production if an incompatible major release is published upstream and a container restart automatically pulls the new image.
**Fix:** Queried the Docker Hub API to find the exact latest semantic tags for each external service (e.g., `certbot/certbot:v5.4.0`, `prom/prometheus:v3.10.0`, `grafana/grafana:12.4.1`) and updated `docker-compose.prod.yml` to use them explicitly.
