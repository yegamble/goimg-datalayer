...
## 2026-05-29 - Pin golangci-lint version
**Issue:** The CI `lint` job fails due to `GOLANGCI_LINT_VERSION: "v2.6.2"`. `v2.x` does not exist.
**Root Cause:** The `v2.x` versions for `golangci-lint` do not exist, throwing a version mismatch error if attempted locally.
**Fix:** Pin `GOLANGCI_LINT_VERSION` to a valid `v1.x` version like `v1.64.5` to avoid installation failure.
