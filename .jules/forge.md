...
## 2024-05-15 - Fix golangci-lint fallback build error
...
**Root Cause:** The `GOLANGCI_LINT_VERSION` environment variable was set to a non-existent `v2.6.2` version, causing the action to fallback to an invalid build instead of using a valid `v1.x` release.
**Fix:** Pinned `GOLANGCI_LINT_VERSION` to the specific valid version `v1.64.5` to prevent invalid fallback builds.
