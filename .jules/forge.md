...
## 2026-08-08 - Redis Healthcheck Authentication Failure
**Issue:** The Redis container in `docker-compose.prod.yml` was failing its healthcheck.
**Root Cause:** The Redis instance requires a password (`--requirepass`), but the default healthcheck command (`redis-cli ping`) does not provide authentication.
**Fix:** Updated the healthcheck command to pass the required password using `redis-cli -a "$$(cat /run/secrets/REDIS_PASSWORD)" ping`.
